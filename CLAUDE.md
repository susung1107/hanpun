...
작업은 항상 `develop` 에서 브랜치를 파서 진행한다. 저장소: https://github.com/susung1107/hanpun · 상세 규칙은 [`docs/git_전략.md`](docs/git_전략.md).

### 브랜치

- 구조: `main`(출시된 것만) ← `develop`(개발 최종, 기본 브랜치) ← 작업 브랜치.
- 이름: `<접두사>/<소문자-영문-케밥케이스>` — 접두사 `feature/`·`fix/`·`refactor/`·`docs/`·`chore/`. 한글·공백·대문자 금지. **브랜치 하나에 작업 하나만.**
- **`main`·`develop` 에 직접 커밋 금지.** 작업 브랜치 → PR(base `develop`) → `--no-ff` 머지 → 브랜치 삭제.
- 흐름: `develop` pull → 브랜치 생성 → 작업 → `pnpm typecheck && lint && test` → push → PR.
- 출시: `develop` → `main` 머지 후 `v1.0.0` 형식 태그.

### 커밋 · 문서

- 커밋 메시지는 `<종류>: <한국어 현재형 요약>` 한 줄. 예: `feature: 홈 화면 예산 진행바 추가`
